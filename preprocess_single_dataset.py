...
def embeddings(X_trans:np.ndarray, y_trans:np.ndarray, classifier:TabPFNClassifier, embedding_strategy:str, embedding_params:dict):
    ...
    if embedding_strategy == 'small_batch_average':
        for _ in range(num_embeddings):
            if 'full_batch' in embedding_params and embedding_params['full_batch']:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, help="If specified, only process this dataset.")
    ...
    preprocess_config_diff_input = config.get('preprocess_config_diff_input', {})
    preprocess_config_pfn_input = config.get('preprocess_config_pfn_input', {})
    ...
    if args.dataset is not None:
        ...
        dataset_list = dataset_csv['dataset name'].tolist()
    # dataset_csv = pd.read_csv(os.path.join(benchmark_base_dir, benchmark_name, benchmark_name + '.csv'))
    for dataset_name in tqdm(dataset_list):
        ...
        try: 
            ...
            print("shape after transform pfn:", y_trans.shape)
        ...
        print("X_trans_diff[:10,0]:", X_trans_diff.iloc[:10,0])
