...
def num_process_nans(dataset: Dataset, policy: Optional[NumNanPolicy]) -> Dataset:
    ...
    for k, v in X_num.items():
        nan_masks[k] = np.array([[False]*v.shape[1]]*v.shape[0])
        print("nan_masks[k].shape:", nan_masks[k].shape)
        for i in range(v.shape[0]):
            for j in range(v.shape[1]):
                val = v[i][j]

                # 统一捕获 float NaN, None, pd.NA
                if pd.isna(val) or isinstance(val, float) and np.isnan(val) or val is None:
                    X_num[k][i][j] = CAT_MISSING_VALUE
                    nan_masks[k][i][j] = True
                # 字符串 "nan"
                elif isinstance(val, str) and val.strip().lower() == "nan":
                    X_num[k][i][j] = CAT_MISSING_VALUE
                    nan_masks[k][i][j] = True
                # 已经是 CAT_MISSING_VALUE
                elif val == CAT_MISSING_VALUE:
                    nan_masks[k][i][j] = True
                else:
                    pass  # 其它值保持不变
    ...
    if any(x.any() for x in nan_masks.values()):  # type: ignore[code]
        if policy is None:
            ...
            try:
                ...
                print(f"SimpleImputer fitting error: {e}")
                imputer = SimpleImputer( strategy=policy)
                imputer.fit(X['train'])
...
def cat_encode(
    ...
    # Step 2. Encode.
        ...
        ohe = sklearn.preprocessing.OneHotEncoder(
            handle_unknown='ignore', dtype=np.float32 # type: ignore[code]
        )
...
def build_target(
    ...
    if policy is None:
        ...
        y = {k: oe.transform(v.reshape(-1, 1)).squeeze(1).astype(np.int64) for k, v in y.items()}
        # Step 2. One-hot encode
        ohe = sklearn.preprocessing.OneHotEncoder(
            handle_unknown='ignore', dtype=np.float32 # type: ignore[code]
        ).fit(y['train'].reshape(-1, 1))
        y = {k: ohe.transform(v.reshape(-1, 1)).astype(np.float32) for k, v in y.items()}
        info['n_classes'] = ohe.categories_[0].shape[0]
...
def transform_dataset(
    ...
    if dataset.X_cat is None:
        ...
        # print("After encoding, X.shape: ", X_cat['train'].shape)
        # print(X_cat['train'][0])
        if is_num:
            X_num = (
                X_cat
            ...
            X_cat = None
        # print("X_num shape: ", X_num['train'].shape)
...
def prepare_fast_dataloader(
    ...
    if not isinstance(y, np.ndarray):
        y = y.toarray()            # csr_matrix → dense numpy array
        y = np.squeeze(y)  
