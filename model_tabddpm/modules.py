...
class MLPDiffusion(nn.Module):
    def __init__(self, d_y, is_y_cond, d_embedding, is_embedding_cond, rtdl_params, dim_t = 128, padding_to: Optional[int] = None, padding_value: Optional[float] = None):
        super().__init__()
        self.dim_t = dim_t
        self.num_classes = d_y
        print("self.num_classes:", self.num_classes)
        self.is_y_cond = is_y_cond
        self.is_embedding_cond = is_embedding_cond

        # d0 = rtdl_params['d_layers'][0]

        rtdl_params['d_in'] = dim_t
        self.d_in_padded = padding_to
        self.padding_value = padding_value if padding_value is not None else 0.0
        rtdl_params['d_out'] = self.d_in_padded 
        ...
        # if is_embedding_cond:
        #     self.embedding_emb = nn.Linear(d_embedding, dim_t)

        self.proj = nn.Linear(self.d_in_padded, dim_t)
    ...
    def forward(self, x, timesteps, y=None, emb=None):
        # x (B, d_in) 
        B, d_in = x.shape
        all_emb = self.time_embed(timestep_embedding(timesteps, self.dim_t))
        if self.is_y_cond and y is not None:
            if self.num_classes > 0:
                y = y.squeeze()
            else:
                y = y.resize(y.size(0), 1).float()
            all_emb += F.silu(self.label_emb(y))
            # label_vec = y.to(self.label_emb.weight.dtype) @ self.label_emb.weight   # [B, 128]
            # all_emb += F.silu(label_vec)
        # if self.is_embedding_cond:
        #     all_emb += F.silu(self.embedding_emb(emb.squeeze()))
        if x.size(1) < self.d_in_padded:
            x = F.pad(x, (0, self.d_in_padded - x.size(1)), value=self.padding_value)
        x = self.proj(x) + all_emb
        output_x =  self.mlp(x)
        return output_x[:, :d_in]
